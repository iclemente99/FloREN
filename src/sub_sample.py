...
def sub_sample(graph, GAS, sampling_size, gene_size, gene_shape, cell_shape, query=False):
    cell_indexs = gene_shape + np.random.choice(np.arange(cell_shape), sampling_size,
                                                replace=False)  #  Generates sampling size number of random indexes for cells 
    sub_matrix = GAS[:, cell_indexs - gene_shape]  #  Substract a samplinz matrix with sampled cells
    if query == True:
        gene_indexs = np.arange(gene_size)
    else:
        gene_indexs = np.nonzero(np.sum(sub_matrix, axis=1))[
            0]  #  Extracts genes indexes with connections on those cells
    #
    sub_matrix = GAS[gene_indexs, :][:,
                 cell_indexs - gene_shape]  #  Substracts a sampling matrix with genes connectec to those sampling cells
    #
    # sub_matrix=norm_rowcol(sub_matrix) # Normalizes the matrix over genes anc ells connections
    #
    _indexs = np.argsort(np.sum(sub_matrix, axis=1))[
              ::-1]  #  Extracts gene indexes based in their connectivity to sampling cells
    ...
    #
    return feature, times, edge_list, indxs, indxs['gene'], indxs['cell']
